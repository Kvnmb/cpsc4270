...
void Camera::Set(int *vp) {
	Set(vp[0], vp[1], vp[2], vp[3]);
}
...
#ifdef __APPLE__
	// Apple "retina coordinates" are half actual pixel coordinates
	arcball.SetCamera(&this->rot, vec2((float) scrnX+scrnW, (float) scrnY+scrnH), minS-100);
#else
...
void Camera::Set(int scrnX, int scrnY, int scrnW, int scrnH, mat4 rot, vec3 tran, float fov, float nearDist, float farDist) {
	// if Apple, scrnW, scrnH in "retina coordinates"
...
void Camera::Resize(int width, int height, bool adjustArcballCenter) {
	// width, height in pixel coordinates
	aspectRatio = (float) width/height;
	persp = Perspective(fov, aspectRatio, nearDist, farDist);
	fullview = persp*modelview;
	if (adjustArcballCenter)
		arcball.SetCamera(&rot, vec2(.5f*width, .5f*height), .5f*(width < height? width : height)-100.f);
}

void Camera::SetSpeed(float tranS) {
	tranSpeed = tranS;
}
...
void Camera::SetRotateCenter(vec3 r) {
	...
	// but orientation applied to scene with new center of rotation shifts the
	// scene origin - this is accommodated by translation offset computed here
	mat4 m = GetRotate();
...
void Camera::Down(double x, double y, bool shift, bool control) {
	Down((int) x, (int) y, shift, control);
}
...
void Camera::Down(int x, int y, bool s, bool c) {
	...
	arcball.Down(x, y, modelview, c); // mOverride = NULL
...
void Camera::Move(vec3 m) {
	MoveTo(m+Position());
}

void Camera::Wheel(double spin, bool shift) {
	if (shift)
		tranOld.z = (tran.z += (float) spin); // dolly in/out
	else
