...
bool MouseOver(double x, double y, vec3 p, mat4 &view, int proximity, int4 vp) {
	if (vp == int4(0,0,0,0)) vp = VP();
	float d = ScreenD(x, y, p, view, NULL, vp);
	return d < proximity;
}
...
bool MouseOver(double x, double y, vec3 p, mat4 &view, int4 vp, int proximity) {
	float d = ScreenD(x, y, p, view, NULL, vp);
	return d < proximity;
}

vec3 *MouseOver(vec3 *points, int npoints, int x, int y, mat4 fullview, int proximity, int4 vp) {
	if (vp == int4(0,0,0,0)) vp = VP();
	for (int i = 0; i < npoints; i++)
		if (::MouseOver(x, y, points[i], fullview, proximity, vp))
			return &points[i];
	return NULL;
}

vec3 *MouseOver(vector<vec3> &points, int x, int y, mat4 fullview, int proximity, int4 vp) {
	return MouseOver(points.data(), points.size(), x, y, fullview, proximity, vp);
}
...
void Mover::Down(vec3 *p, int x, int y, mat4 modelview, mat4 persp) {
	mat4 inv = Inverse(modelview); // probably could do this by separating into rot and tran matrices
...
void Mover::Down(mat4 *t, int x, int y, mat4 modelview, mat4 persp) {
	...
	transform = t;
	point = NULL;
	pMousedown = lastMouse = p; // MatrixOrigin(*t);
	SetPlane(p, modelview, plane);
}
...
mat4 Framer::Drag(int x, int y, mat4 modelview, mat4 persp) {
	if (moverPicked) {
		vec3 dt = mover.Drag(x, y, modelview, persp);
		arcball.SetCenter(ScreenPoint(MatrixOrigin(*arcball.m), persp*modelview));
		return Translate(dt);
	}
	else
		return arcball.Drag(x, y);
}
...
void Arcball::Down(int x, int y, mat4 modelview, bool constrain, mat4 *mOverride) {
	modelviewInverse = Inverse(modelview);
	constraint = { ConstraintIndex::None, vec3() };
...
mat4 Arcball::Drag(int x, int y) {
	...
	if (dot(axis, axis) > .000001f) {
		if (use == Use::Body)
			...
			// need axis in object space, not camera space
		...
		if (m != NULL) {
			mat4 mprev = *m, mprevinv = Inverse(mprev);
			mat3 m3 = qq.Get3x3(), m3scale = scale*m3;
			for (int i = 0; i < 3; i++)
				for (int j = 0; j < 3; j++)
					(*m)[i][j] = m3scale[i][j];
			return (*m)*mprevinv;
				// *m was updated with ab initio 3x3 matrix from qrot
				// qrot is rotation since Down, not since last call to Drag
				// thus, to return difference matrix to apply to children
				// want mdif such that mcurrent = mdif*mprev
		}
	}
	return mat4(1);
}
...
void Arcball::Freeze(bool f) { freeze = f; }
...
void Arcball::Draw(bool showConstrainAxes, mat4 *matOverride, vec3 *colorOverride) {
	...
	if (mm) {
		...
		float l = length(mouseDown-mouseMove);
...
vec3 WidgetProjectToLine(vec3 &p, vec3 &p1, vec3 &p2) {
...
Joystick::Joystick(vec3 *b, vec3 *v, float arrowScale) :
	base(b), vec(v), arrowScale(arrowScale) {
	len = v? length(*v) : 0;
}

bool Joystick::Hit(int x, int y, vec3 b, vec3 v, mat4 fullview, int4 vp) {
	if (vp == int4(0,0,0,0)) vp = VP();
	return ScreenDSq(x, y, b, fullview, NULL, vp) < 100 ||
		   ScreenDSq(x, y, b+v, fullview, NULL, vp) < 100;
}

bool Joystick::Hit(int x, int y, mat4 fullview, int4 vp) {
	if (vp == int4(0,0,0,0)) vp = VP();
	return ScreenDSq(x, y, *base+arrowScale**vec, fullview, NULL, vp) < 100;
}

void Joystick::Down(int x, int y, mat4 modelview, mat4 persp, int4 vp) {
	if (vp == int4(0,0,0,0)) vp = VP();
	mat4 fullview = persp*modelview;
	fwdFace = FrontFacing(*base, arrowScale**vec, fullview);
	mode = ScreenDSq(x, y, *base, fullview, NULL, vp) < 100? JoyType::A_Base :
		   ScreenDSq(x, y, *base+arrowScale**vec, fullview, NULL, vp) < 100? JoyType::A_Tip : JoyType::A_None;
	if (mode == JoyType::A_Base)
		SetPlane(*base, modelview, plane);
}

void Joystick::Down(int x, int y, vec3 *b, vec3 *v, mat4 modelview, mat4 persp, int4 vp) {
	if (vp == int4(0,0,0,0)) vp = VP();
	base = b;
	vec = v;
	Down(x, y, modelview, persp, vp);
}

void Joystick::Drag(int x, int y, mat4 modelview, mat4 persp, int4 vp) {
	if (vp == int4(0,0,0,0)) vp = VP();
	vec3 p1, p2;                                        // p1p2 is world-space line that xforms to line perp to screen at (x, y)
	ScreenLine((float) x, (float) y, modelview, persp, p1, p2, vp);
	if (mode == JoyType::A_Base) {
	...
	if (mode == JoyType::A_Tip) {
		vec3 hit1, hit2, v;
		int nhits = LineSphere(p1, p2, *base, len*arrowScale, hit1, hit2);
		if (nhits == 0) {
			vec3 n = WidgetProjectToLine(*base, p1, p2);
			v = normalize(n-*base);
		}
		else {
			v = hit2-*base;
...
void Joystick::Draw(mat4 modelview, mat4 persp, vec3 baseColor, vec3 lineColor, vec3 headColor) {
	mat4 save = GetDrawView();
//  bool frontFacing = FrontFacing(*base, *vec, modelview);
	UseDrawShader(persp*modelview);
//  ArrowV(base, len*v, modelview, color, NULL, 10);
	Line(*base, *base+arrowScale**vec, 5, lineColor);
//	Disk(*base, 12, baseColor);
//	Disk(*base+arrowScale**vec, 12, headColor);
	SetDrawView(save);
}
...
void Button::Initialize(string imageFilename) {
	ReadTexture(imageFilename.c_str(), &textureName, true, &nTexChannels);
}
...
void Button::Draw(int textureUnit) {
	...
	vec3 p1((float)x,(float)y,0), p2((float)x,(float)y2,0), p3((float)x2,(float)y2,0), p4((float)x2,(float)y,0);
	UseDrawShader(ScreenMode());
	Quad(p1, p2, p3, p4, textureName, textureUnit, 1, nTexChannels);
}
...
Toggler::Toggler(const char *cname, bool on, int x, int y, float dia, vec3 onCol, vec3 offCol, vec3 ringCol)
	: on(on), x(x), y(y), dia(dia), onCol(onCol), offCol(offCol), ringCol(ringCol) {
	ptr = &this->on;
	if (cname) name = string(cname);
};
...
//	float f = length(vec2((float)(x), (float)(y))-vec2(xMouse, yMouse));
//	bool hit = f < proximity;
//	return hit;
}
...
bool Toggler::DownHit(float xMouse, float yMouse, int proximity) {
	...
	if (hit) {
		if (ptr != NULL) *ptr = (*ptr == true)? false : true;
		else on = !on;
	}
	return hit;
}
