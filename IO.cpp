...
void LoadTexture(unsigned char *pixels, int width, int height, int bpp, unsigned int textureName, bool bgr, bool mipmap) {
	...
	if (bpp == 1)
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width, height, 0, GL_RED, GL_UNSIGNED_BYTE, temp);
	if (mipmap) {
...
bool ReadTexture(const char *filename, GLuint *textureName, bool mipmap, int *n, int *w, int *h) {
	...
	if (h) *h = height;
	glGenTextures(1, textureName);
	*textureName = LoadTexture(data, width, height, nChannels, false, mipmap);
...
void SaveJpg(const char *filename, int quality) {
	int width, height;
	unsigned char *cPixels = GetData(width, height);
	stbi_flip_vertically_on_write(true);
	stbi_write_jpg(filename, width, height, 3, cPixels, quality);
	delete [] cPixels;
}
...
int ReadGIF(const char *filename, vector<GLuint> &textureNames, int *nChannels, vector<float> *frameDurations) {
	...
	for (int i = 0; i < nFrames; i++) {
		unsigned char *p = pdata+i*width*height*nChan;
		LoadTexture(p, width, height, nChan, textureNames[i], false, true);
	}
